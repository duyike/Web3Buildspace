...
const exchange_name = "DETH/ETH Exchange";

const token_name = "DETH";
...
async function getTokenEthRate(maxSlippagePct) {
  let poolState = await getPoolState();
  return {
    maxExchangeRate: Math.round(
      poolState.token_eth_rate *
        Number(1 + maxSlippagePct / 100) *
        rate_denominator
    ),
    minExchangeRate: Math.round(
      poolState.token_eth_rate *
        Number(1 - maxSlippagePct / 100) *
        rate_denominator
    ),
...
async function addLiquidity(amountEth, maxSlippagePct) {
  let { maxExchangeRate, minExchangeRate } = await getTokenEthRate(
    maxSlippagePct
  );

  let maxAmountToken = maxExchangeRate * amountEth;
  await token_contract.approve(exchange_address, maxAmountToken);

  await exchange_contract.addLiquidity(maxExchangeRate, minExchangeRate, {
...
async function removeLiquidity(amountEth, maxSlippagePct) {
  ...
  await exchange_contract.removeLiquidity(
    amountEth,
    maxExchangeRate,
    minExchangeRate
  );
...
async function removeAllLiquidity(maxSlippagePct) {
  ...
  await exchange_contract.removeAllLiquidity(maxExchangeRate, minExchangeRate);
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  await exchange_contract.swapTokensForETH(amountToken, maxExchangeRate);
}

async function swapETHForTokens(amountEth, maxSlippagePct) {
  let poolState = await getPoolState();
  let maxExchangeRate = Math.round(
    poolState.eth_token_rate * (1 + maxSlippagePct / 100) * rate_denominator
  );

  await exchange_contract.swapETHForTokens(maxExchangeRate, {
    value: ethers.utils.parseUnits(amountEth.toString(), "wei"),
  });
}

function addressLiquidity() {
  return exchange_contract.addressLiquidity(defaultAccount).then((response) => {
    return ethers.utils.formatUnits(response) * 1000000000;
  });
}
